...
- [Rulebricks Application](#rulebricks-application)
- [Database (Supabase)](#database-supabase)
- [Message Queue (Kafka)](#message-queue-kafka)
- [Ingress (Traefik)](#ingress-traefik)
- [Autoscaling (KEDA)](#autoscaling-keda)
- [Certificates (cert-manager)](#certificates-cert-manager)
- [Logging (Vector)](#logging-vector)
...
Global settings are shared across all components and subcharts. These are the most important values to configure for your deployment.
...
> **Important:** The domain must resolve to your cluster's load balancer before enabling TLS. See [DNS Configuration](#dns-external-dns) for automatic setup.
...
| `rulebricks.app.image.tag`        | string | `"1.2.8"`                          | Image version tag       |
...
| `rulebricks.hps.replicas`         | integer | `2`     | Number of HPS gateway replicas |
...
| `rulebricks.hps.workers.keda.maxReplicaCount` | integer | `8`     | Maximum workers               |
...
| `rulebricks.hps.workers.keda.lagThreshold`    | integer | `8`     | Kafka lag threshold           |
...
kafka:
  overrideConfiguration:
    ...
    num.partitions: "3" # Increase for parallelism
  controller:
    extraEnvVars:
