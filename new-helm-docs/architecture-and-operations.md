# Architecture & Operations Guide

This document explains the Rulebricks Helm chart architecture, component interactions, migration flows, and operational considerations.

## Table of Contents

- [System Architecture](#system-architecture)
- [Installation Flows](#installation-flows)
- [Database Migrations](#database-migrations)
- [DNS and TLS Setup](#dns-and-tls-setup)
- [Scaling Considerations](#scaling-considerations)
- [Troubleshooting](#troubleshooting)

---

## System Architecture

### Component Overview

```mermaid
graph TB
    subgraph Internet
        Users[Users/API Clients]
    end
    
    subgraph Kubernetes["Kubernetes Cluster"]
        subgraph Ingress["Ingress Layer"]
            Traefik[Traefik LoadBalancer]
        end
        
        subgraph Core["Rulebricks Core"]
            App[Rulebricks App]
            HPS[HPS Gateway]
            Workers[HPS Workers]
            Redis[Redis Cache]
        end
        
        subgraph Data["Data Layer"]
            Kafka[Kafka]
            Postgres[(PostgreSQL)]
            Kong[Kong API Gateway]
        end
        
        subgraph Supabase["Supabase Stack"]
            Auth[GoTrue Auth]
            Rest[PostgREST]
            Realtime[Realtime]
            Storage[Storage API]
        end
        
        subgraph Support["Supporting Services"]
            Vector[Vector]
            CertManager[cert-manager]
            KEDA[KEDA]
            ExternalDNS[external-dns]
        end
    end
    
    subgraph External["External Services"]
        LetsEncrypt[Let's Encrypt]
        DNS[DNS Provider]
        LogSink[Log Sink S3/etc]
    end
    
    Users --> Traefik
    Traefik --> App
    Traefik --> HPS
    Traefik --> Kong
    
    App --> Redis
    App --> Kafka
    HPS --> Workers
    Workers --> Kafka
    Workers --> Redis
    
    Kong --> Auth
    Kong --> Rest
    Kong --> Realtime
    Kong --> Storage
    
    Auth --> Postgres
    Rest --> Postgres
    
    Kafka --> Vector
    Vector --> LogSink
    
    CertManager --> LetsEncrypt
    ExternalDNS --> DNS
    KEDA --> Workers
```

### Request Flow

```mermaid
sequenceDiagram
    participant Client
    participant Traefik
    participant HPS
    participant Worker
    participant Kafka
    participant Redis
    participant App
    
    Client->>Traefik: POST /api/v1/solve/{rule}
    Traefik->>HPS: Route to HPS Gateway
    HPS->>Kafka: Publish solve request
    HPS->>Client: 202 Accepted (async)
    
    Kafka->>Worker: Consume message
    Worker->>Redis: Fetch rule definition
    Worker->>Worker: Execute rule
    Worker->>Kafka: Publish result
    Worker->>Kafka: Publish decision log
    
    Note over Client,App: For sync requests
    Client->>Traefik: POST /api/v1/solve/{rule}?sync=true
    Traefik->>HPS: Route to HPS
    HPS->>Worker: Direct execution
    Worker->>Redis: Fetch rule
    Worker->>Worker: Execute
    Worker->>HPS: Return result
    HPS->>Client: 200 OK + result
```

### Component Responsibilities

| Component | Purpose | Scaling |
|-----------|---------|---------|
| **Traefik** | Ingress, TLS termination, routing | HPA (1-2 replicas default) |
| **App** | Web UI, rule editor, management API | Single replica |
| **HPS Gateway** | API gateway for rule execution | Static replicas |
| **HPS Workers** | Rule execution engines | KEDA autoscaling |
| **Redis** | Rule cache, session storage | Single replica with persistence |
| **Kafka** | Message queue, event streaming | KRaft mode, single node default |
| **PostgreSQL** | Persistent data storage | Single replica with PVC |
| **Vector** | Log aggregation and forwarding | Static replicas |

---

## Installation Flows

### Standard Two-Phase Installation

The default installation flow when not using external-dns:

```mermaid
flowchart TD
    A[helm install] --> B{TLS Enabled?}
    B -->|No| C[Deploy without TLS]
    C --> D[Get LoadBalancer IP]
    D --> E[Create DNS Records Manually]
    E --> F[Wait for DNS Propagation]
    F --> G[helm upgrade --set global.tlsEnabled=true]
    G --> H[cert-manager requests certificate]
    H --> I[Let's Encrypt validates HTTP-01]
    I --> J[Certificate issued]
    J --> K[Installation Complete]
    
    B -->|Yes| L{DNS Records Exist?}
    L -->|Yes| H
    L -->|No| M[Certificate validation fails]
    M --> N[Retry until DNS propagates]
    N --> H
```

### Single-Phase with external-dns

When using automatic DNS management:

```mermaid
flowchart TD
    A["helm install<br/>--set external-dns.enabled=true<br/>--set global.externalDnsEnabled=true<br/>--set global.tlsEnabled=true"] --> B[Deploy all components]
    B --> C[Traefik creates LoadBalancer]
    C --> D[external-dns detects ingress annotations]
    D --> E[DNS records created automatically]
    E --> F[cert-manager requests certificate]
    F --> G[Let's Encrypt validates HTTP-01]
    G --> H{Validation Success?}
    H -->|Yes| I[Certificate issued]
    H -->|No| J[Retry with backoff]
    J --> G
    I --> K[Installation Complete]
```

### Timing Considerations

| Phase | Typical Duration | Notes |
|-------|-----------------|-------|
| LoadBalancer provisioning | 1-3 minutes | Cloud provider dependent |
| external-dns record creation | 1-2 minutes | Polling interval |
| DNS propagation | Seconds to minutes | Provider dependent |
| Certificate issuance | 30 seconds - 2 minutes | After DNS resolves |

> **Note:** cert-manager automatically retries failed challenges. The system is eventually consistent—expect 3-10 minutes for full stabilization when using external-dns.

---

## Database Migrations

The chart includes two migration job variants that run automatically based on your configuration.

### Migration Decision Tree

```mermaid
flowchart TD
    A[Helm Install/Upgrade] --> B{supabase.enabled?}
    B -->|true| C[Self-Hosted Migration Job]
    B -->|false| D{accessToken provided?}
    D -->|Yes| E[Managed Supabase Setup Job]
    D -->|No| F[No migration job runs]
    
    C --> G[Wait for Postgres ready]
    G --> H[Extract migrations from app image]
    H --> I[Apply SQL migrations via psql]
    I --> J[Track in schema_migrations table]
    
    E --> K[Install Supabase CLI]
    K --> L[Authenticate with access token]
    L --> M[Link to project]
    M --> N[Push config.toml]
    N --> O[Run db push]
    O --> P[Enable SSL enforcement]
```

### Self-Hosted Migration Job

**Triggered when:** `supabase.enabled: true`

**Process:**

1. **Init Container** extracts Supabase assets from the Rulebricks app image
2. **Migration Container** uses `postgres:15-alpine` to:
   - Wait for PostgreSQL to be ready
   - Create `schema_migrations` tracking table
   - Apply each `.sql` file in order
   - Record applied migrations to prevent re-runs

**Key Details:**

```yaml
# Job naming pattern
name: {{ .Release.Name }}-db-migrate-{{ .Release.Revision }}

# Runs on
annotations:
  "helm.sh/hook": post-install,post-upgrade
  "helm.sh/hook-delete-policy": before-hook-creation

# Auto-cleanup
spec:
  ttlSecondsAfterFinished: 600  # 10 minutes
```

**Migration Tracking:**

```sql
CREATE TABLE IF NOT EXISTS schema_migrations (
  version VARCHAR(255) PRIMARY KEY,
  applied_at TIMESTAMP DEFAULT NOW()
);
```

Each migration file is tracked by filename. Already-applied migrations are skipped.

### Managed Supabase Setup Job

**Triggered when:** `supabase.enabled: false` AND `global.supabase.accessToken` is provided

**Process:**

1. **Init Container** extracts Supabase project configuration
2. **Setup Container** uses `node:20-alpine` to:
   - Install Supabase CLI
   - Authenticate with your access token
   - Link to your Supabase project
   - Download and configure email templates
   - Push configuration (SMTP, auth settings)
   - Run `supabase db push` to apply schema
   - Enable SSL enforcement

**Configuration Flow:**

```mermaid
flowchart LR
    A[config.example.toml] --> B[Variable Replacement]
    B --> C[config.toml]
    
    D[Email Template URLs] --> E[Download Templates]
    E --> F[Local Template Files]
    
    C --> G[supabase config push]
    F --> G
    G --> H[Supabase Project Updated]
```

**Variables Replaced:**

| Placeholder | Source |
|-------------|--------|
| `env(FULL_URL)` | `https://{global.domain}` |
| `env(SMTP_HOST)` | `global.smtp.host` |
| `env(SMTP_PORT)` | `global.smtp.port` |
| `env(SMTP_USER)` | `global.smtp.user` |
| `env(SMTP_PASS)` | `global.smtp.pass` |
| `env(EMAIL_SUBJECTS_*)` | `global.supabase.emails.subjects.*` |
| `env(EMAIL_TEMPLATES_*)` | Downloaded from URLs |

### Debugging Migrations

**Self-hosted:**
```bash
# Check job status
kubectl get jobs -n rulebricks -l app.kubernetes.io/component=migrations

# View logs
kubectl logs job/rulebricks-db-migrate-1 -n rulebricks

# Check migration table
kubectl exec -it deploy/rulebricks-supabase-db -n rulebricks -- \
  psql -U postgres -c "SELECT * FROM schema_migrations ORDER BY applied_at;"
```

**Managed Supabase:**
```bash
# Check job status
kubectl get jobs -n rulebricks -l app.kubernetes.io/component=managed-supabase-setup

# View logs
kubectl logs job/rulebricks-managed-supabase-setup-1 -n rulebricks
```

---

## DNS and TLS Setup

### DNS Record Requirements

| Scenario | Records Needed |
|----------|----------------|
| Self-hosted Supabase | `{domain}`, `supabase.{domain}` |
| Managed Supabase | `{domain}` only |

### TLS Certificate Flow

```mermaid
sequenceDiagram
    participant Helm
    participant CertManager as cert-manager
    participant Issuer as ClusterIssuer
    participant LE as Let's Encrypt
    participant Traefik
    
    Helm->>CertManager: Create Certificate resource
    CertManager->>Issuer: Request certificate
    Issuer->>LE: ACME challenge request
    LE->>Traefik: HTTP-01 challenge (/.well-known/acme-challenge/*)
    Traefik->>LE: Challenge response
    LE->>Issuer: Certificate issued
    Issuer->>CertManager: Store in Secret
    CertManager->>Traefik: TLS secret available
    Traefik->>Traefik: Enable HTTPS
```

### external-dns Annotations

When `global.externalDnsEnabled: true`, these annotations are added to ingresses:

```yaml
annotations:
  external-dns.alpha.kubernetes.io/hostname: rulebricks.example.com
  external-dns.alpha.kubernetes.io/ttl: "300"
```

external-dns watches for these annotations and creates corresponding DNS records.

---

## Scaling Considerations

### Horizontal Scaling

| Component | Scaling Method | Trigger |
|-----------|---------------|---------|
| Traefik | HPA | CPU utilization |
| HPS Workers | KEDA | Kafka lag, CPU |
| Vector | Manual | Log volume |

### KEDA Scaling Configuration

```mermaid
flowchart LR
    A[Kafka Consumer Lag] --> B{lag > threshold?}
    B -->|Yes| C[Scale Up Workers]
    B -->|No| D{CPU > threshold?}
    D -->|Yes| C
    D -->|No| E[Maintain Current]
    
    F[Cooldown Period] --> G{No scale triggers<br/>for 5 min?}
    G -->|Yes| H[Scale Down]
    G -->|No| E
```

### Resource Sizing Guide

| Workload | HPS Workers | Redis Memory | Kafka Storage |
|----------|-------------|--------------|---------------|
| Development | 2-4 | 256Mi | 5Gi |
| Small (< 100 req/s) | 4-8 | 1Gi | 10Gi |
| Medium (100-1000 req/s) | 8-16 | 4Gi | 50Gi |
| Large (> 1000 req/s) | 16-32+ | 8Gi+ | 100Gi+ |

### Kafka Partitioning

For high throughput, increase Kafka partitions:

```yaml
kafka:
  overrideConfiguration:
    num.partitions: "12"  # Match max worker count
```

---

## Troubleshooting

### Common Issues

#### Certificate Not Issuing

```bash
# Check certificate status
kubectl get certificates -n rulebricks
kubectl describe certificate rulebricks-tls -n rulebricks

# Check cert-manager logs
kubectl logs -n cert-manager -l app=cert-manager

# Check challenges
kubectl get challenges -n rulebricks
```

**Common causes:**
- DNS not pointing to LoadBalancer
- Port 80 blocked by firewall
- Rate limiting from Let's Encrypt

#### Migration Job Failing

```bash
# Check job status
kubectl get jobs -n rulebricks

# Get detailed error
kubectl describe job rulebricks-db-migrate-1 -n rulebricks

# Check pod logs
kubectl logs job/rulebricks-db-migrate-1 -n rulebricks --all-containers
```

**Common causes:**
- Database not ready (increase readiness wait)
- Invalid credentials
- Network policy blocking access

#### HPS Workers Not Scaling

```bash
# Check KEDA
kubectl get scaledobject -n rulebricks
kubectl describe scaledobject rulebricks-hps-workers -n rulebricks

# Check Kafka metrics
kubectl exec -it rulebricks-kafka-0 -n rulebricks -- \
  kafka-consumer-groups.sh --bootstrap-server localhost:9092 \
  --describe --group hps-workers
```

**Common causes:**
- Kafka consumer group not found
- KEDA unable to reach Kafka
- Incorrect threshold configuration

### Health Checks

```bash
# Overall status
kubectl get pods -n rulebricks

# Check specific component
kubectl describe pod -n rulebricks -l app.kubernetes.io/name=rulebricks

# View events
kubectl get events -n rulebricks --sort-by='.lastTimestamp'
```

### Log Collection

```bash
# All pods in namespace
kubectl logs -n rulebricks -l app.kubernetes.io/instance=rulebricks --all-containers

# Specific component with follow
kubectl logs -n rulebricks -l app.kubernetes.io/component=hps-worker -f
```

