...
To create a release, click the "New Release" button in the Releases tab. The modal that appears guides you through the essential decisions: which environment should receive this release, which asset you want to release, and which version of that asset should be deployed.

![Creating a new release requires selecting an environment, a published asset, and a specific version](images/new-release-modal.png)
...
The version selection lets you choose which specific published version to release. This is particularly important when you have made multiple iterations on a rule and want to release a specific checkpoint rather than the latest version. Each time you publish a rule, Rulebricks creates a new version snapshot, and the version selector shows you all available versions along with their current release status.

The notes field provides an opportunity to document why this release is being created, what changes it contains, or what the approvers should pay attention to during review. These notes become part of the release's permanent record and appear in the activity feed for anyone reviewing the release.
...
![Filtering releases by environment helps you focus on what matters for each deployment target](images/select-release-environment.png)
...
![The release detail view shows approval status, the release asset, and a collaborative activity feed](images/comment-markdown.png)
...
The promote action appears on released assets and shows only environments at the next level in your hierarchy. Promotion creates a new release in the target environment that references its origin, providing a clear audit trail of how changes flow through your deployment pipeline. The promoted release still requires approval according to the target environment's configuration, ensuring that each stage maintains its own governance.
