...
The Continue If node allows you to proceed based on one of three selectable conditions: if a property is "truthy" (not empty, non-zero, non-null), "true" (exactly a boolean that equals true), or "false" (exactly a boolean that equals false). If the condition is met, the flow will continue along the edges exiting the node. This flow function is useful if you would like to only execute specific branches of rules/actions. Simply use two "Continue If" nodes to model if/else logic. __Note that all data you would like to proceed with in your flow after this node must be wired in as inputs.__
