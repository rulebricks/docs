...
| `rulebricks.hps.limits.itemMaxBytes`                   | `6291456`   | `ITEM_MAX_BYTES`                 | Serialized cap per single payload inside a bulk array (6 MiB). Must stay below the topics' `max.message.bytes` minus envelope headroom.       |
| `rulebricks.hps.limits.contextBatchBodyLimitBytes`     | `134217728` | `CONTEXT_BATCH_BODY_LIMIT_BYTES` | NDJSON body cap for `contexts/batch` (128 MiB).                                                                                               |
| `rulebricks.hps.limits.contextBatchMaxItems`           | `100000`    | `CONTEXT_BATCH_MAX_ITEMS`        | Records per context batch request.                                                                                                            |
...
| `rulebricks.hps.limits.chunkHardMaxBytes`              | `2097152`   | `CHUNK_HARD_MAX_BYTES`           | Hard per-chunk input cap (2 MiB), leaving 4x response-amplification headroom under the 8 MiB Kafka message cap.                               |
| `rulebricks.hps.limits.responseFetchMaxBytes`          | `16777216`  | `RESPONSE_MAX_BYTES_PER_PARTITION` | Gather-side consumer fetch ceiling per partition; keep at ~2x the Kafka message cap.                                                        |
| `rulebricks.hps.limits.workerFetchMaxBytes`            | `16777216`  | `WORKER_MAX_BYTES_PER_PARTITION` | Worker request-consumer fetch ceiling per partition; same 2x rule.                                                                            |
...
| `CONTEXT_CASCADE_MAX_INSTANCES`  | `2000`  | gateway         | Max dependent (parent) instances re-evaluated per relationship when a context batch cascades into related contexts. Bounds worst-case ingest tail latency; excess is reported with `truncated: true`.       |
| `CONTEXT_CASCADE_BUDGET_MS`      | `45000` | gateway         | Wall-clock budget for the whole cascade phase of one batch request. Once spent, remaining relationships are reported as deferred (`reason: cascade_budget_exceeded`) instead of running unbounded.           |
...
The RPC topics (`solution`, `solution-response`) default to an 8 MiB `max.message.bytes`, matching the broker's `message.max.bytes`, so worker responses keep 4x amplification headroom over the 2 MiB request chunks. The `logs` topic stays at 2 MB. If you lower these, lower `rulebricks.hps.limits` (chunk sizes, item cap) to match.
