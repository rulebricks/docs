...
| `global.ai.openaiBaseUrl` | string  | `""`    | Optional OpenAI-compatible API base URL, including `/v1` |
...
| `rulebricks.hps.warmup.*`                   | object  | enabled, 600s TTL                                                             | Holds the worker fleet at its ceiling before large batch workloads              |
| `rulebricks.hps.imagePrepull.enabled`       | boolean | `true`                                                                        | Pre-pulls HPS server and worker images on nodes before burst scale-out          |
| `rulebricks.hps.extraEnv`                   | list    | context reuse and cascade guard defaults                                      | Advanced HPS runtime environment overrides                                      |
| `rulebricks.hps.workers.extraEnv`           | list    | context reuse default                                                         | Advanced worker runtime environment overrides                                   |
...
| `rulebricks.hps.keda.minReplicaCount`    | integer | `4`     | Baseline gateway fleet                  |
| `rulebricks.hps.keda.maxReplicaCount`    | integer | `16`    | Gateway scale-out ceiling               |
...
| `rulebricks.hps.keda.lagThreshold`       | integer | `50`    | `solution-response` lag threshold       |
| `rulebricks.hps.keda.cpuThreshold`       | integer | `45`    | CPU utilization target (%)              |
...
| `kafka.version`            | string  | `"4.2.1"`  | Kafka version supported by the bundled Strimzi operator       |
| `kafka.replicas`           | integer | `1`        | Broker replica count                                          |
| `kafka.storage.size`       | string  | `"50Gi"`   | Broker persistent storage size                                |
...
| `kafkaBridge.image`                  | string  | `"docker.io/rulebricks/kafka-proxy:0.4.3-r1"` | kafka-proxy sidecar image (override for registry mirrors) |
...
| `traefik.autoscaling.maxReplicas` | integer | `8`         | Maximum replicas   |
...
| `clickstack.collector.filters.*` | Trace and log noise controls applied before ClickStack storage                         |
