...
graph TB
    ...
    Vector --> ClickHouse
    ObjectStorage -.->|bounded fallback / PVC-less| ClickHouse
    App --> ClickHouse
    App --> ClickStack
    HPS --> ClickStack
...
| **Vector**      | Decision log dual-write and forwarding   | Static replicas                       |
| **ClickHouse**  | Bounded decision log queries and telemetry | Single replica, PVC by default      |
...
Every rule execution emits a structured decision log. HPS API and worker pods produce entries to the Kafka `logs` topic (`com.rulebricks.logs` with the default topic prefix). In persistent mode, Vector writes each normalized record both to the TTL-bounded ClickHouse MergeTree table used by the app and to the durable zstd NDJSON archive:
...
Normal in-app queries use only the table. The bucket is read for bounded execution-trace fallback and in PVC-less mode, and remains available to your warehouse for full-history analysis. The full walkthrough, values, and custom sink examples live in [Decision Logs](/private-deployment/decision-logs). Database backups travel to the same bucket under a separate key prefix; see [Storage & Backups](/private-deployment/storage-and-backups).
