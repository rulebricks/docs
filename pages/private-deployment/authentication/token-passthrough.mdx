...
- A user's tokens can carry different `sub` values over time (e.g., if the upstream system rotates identifiers) with no impact, because emails are the matching key.
