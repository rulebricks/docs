---
metaDescription: Developer guide for customizing the Rulebricks CLI for your organization. Learn about the codebase structure, customization points, and how to fork the repository.
---
...
<Callout>
  This guide is intended for technical audiences who want to customize the CLI for their own company's deployment needs. If you're just deploying Rulebricks, see the [Getting Started](./getting-started) guide instead.
</Callout>
...
**Location:** `src/operations.go` or modify Helm chart values (charts are downloaded at runtime)
...
3. **Add Helm chart** (if needed):
   - Create chart in your own repository or locally
   - Modify `ChartManager` to download from your repository
...
**Location:** Charts are downloaded at runtime. To customize:

1. **Extract and modify cached charts:**
   ```bash
   cd ~/.rulebricks/charts/
   tar -xzf rulebricks-*.tgz
   # Modify templates in the extracted directory
...
**Example:** Adding a custom ConfigMap via custom values
```yaml
# In rulebricks.yaml
advanced:
  custom_values:
