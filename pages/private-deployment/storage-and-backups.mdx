...
- **Decision logs** under `paths.decisionLogs`: the durable, full-history zstd NDJSON archive written by Vector in hourly Hive partitions. ClickHouse reads it only for bounded execution-trace fallback or in PVC-less mode; normal persistent-mode app queries use the local MergeTree table. See [Decision Logs](/private-deployment/decision-logs).
