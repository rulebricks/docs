...
- **Context batch requests** (`POST /api/v1/contexts/batch/{slug}`, self-hosted bulk mode) share the same body ceiling and additionally cap records per request at 10,000 by default (HTTP 413 above it; configurable via `CONTEXT_BATCH_MAX_ITEMS` on HPS). Database writes are chunked at 500 records per statement (`CONTEXT_BATCH_DB_CHUNK`). See [Bulk Ingestion](/contexts/bulk-ingestion) for the endpoint contract.
