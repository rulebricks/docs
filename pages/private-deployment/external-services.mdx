...
Redis sits in front of Supabase as a shared cache layer. API key authentication payloads are cached with a short TTL (60 seconds), while rule/flow definitions and named-environment release pins are cached without expiry and invalidated over a Redis pub/sub channel when they change; every pod also keeps an in-process LRU in front of Redis. Redis is strictly a cache: release pins and version content are authoritatively stored in Postgres, and solvers self-heal missing Redis entries from the database on demand. In practice, Supabase sees very few direct queries under normal operation.

## Externalizing Kafka

### How Rulebricks Uses Kafka

Rulebricks uses three Kafka topics. Topic names carry the configured `rulebricks.app.logging.kafkaTopicPrefix` (default `com.rulebricks.`, so the actual topic is `com.rulebricks.solution` and so on). Consumer group IDs carry the same prefix (e.g. `com.rulebricks.generic-workers`): Kafka group membership and rebalancing are group-wide rather than per-topic, so two Rulebricks deployments sharing one cluster must join distinct groups. Otherwise every deploy of one stalls the other's consumers. The prefix exists so neither topics nor groups collide on shared or managed clusters; set it to `""` to disable prefixing of both.
...
Set `max.message.bytes` to `33554432` (32 MiB) on `solution` and `solution-response`. Request chunks are normally planner-capped at 2 MiB of input, while a legal 20 MiB single item occupies its own chunk. A worker result rides as one Kafka message and may grow when rules expand payloads; a result above the cap fails with an explicit HTTP 413 rather than a timeout.

Set the `logs` topic to `max.message.bytes=67108864` (64 MiB). A full-fidelity single-item decision log stores both request and response as JSON strings inside a second JSON envelope, so a 40 MiB nominal pair needs additional encoding headroom. The broker must use `message.max.bytes >= 67108864`, `replica.fetch.max.bytes >= 134217728`, and, where exposed, `replica.fetch.response.max.bytes >= 134217728`. The chart's bridge hook reconciles topic caps but cannot change these broker settings.

Managed-service caveats for this profile:

- **Provisioned/self-managed Kafka** works when its broker and topic limits are configured before installation. For provisioned MSK, attach a custom cluster configuration with the broker settings above.
- **AWS MSK Serverless** tops out at 8 MiB and cannot carry this profile.
- **Azure Event Hubs** tiers top out below 64 MiB and cannot carry full-fidelity decision logs for 20 MiB payloads.
- **Confluent Cloud** tiers currently top out below 64 MiB and cannot carry this profile.
- **GCP Managed Kafka** defaults to 10 MiB. Raise the broker/topic settings out of band and verify that the selected service tier permits 64 MiB records.

For a provider with a lower hard limit, override the complete HPS/Kafka/Vector size envelope rather than changing one cap in isolation.
...
<Callout type="warning">
  ...
  common cause of solve requests timing out at the full RPC deadline.
</Callout>
...
HPS does not care how your Kafka cluster manages metadata. It never connects to ZooKeeper directly; it only speaks the Kafka wire protocol to brokers via a bootstrap address. ZooKeeper-backed and KRaft-mode clusters work identically, but the selected service tier must also support the chart's message-size envelope.
...
External Kafka must be tuned for the 64 MiB broker/log cap, 32 MiB execution topics, and 128 MiB replica fetches described above. Also ensure the three topics have the correct partition counts and reasonable ISR settings (for example, `min.insync.replicas=1` for a single broker or `2` for multi-broker production).

### Request Size Limits

Admission is byte-first; item counts are not limited by default. The Helm chart ships a **large-batch profile** sized for 100MB-class bulk requests; every bound below is an env var on HPS (chart value in `rulebricks.hps.limits`), and the Kafka message caps, ingress timeouts, and HPS memory are sized in lockstep with them. Useful when setting client expectations:

- **Total request body:** 128 MiB chart default (HTTP 413 above it; `HTTP_BODY_LIMIT_BYTES`). Without the chart env, the HPS binary defaults to 6 MiB. Clients get the best latency profile around 1 MB or less; larger bodies execute fine but cost proportionally more parse time and fan out into more chunks.
- **Single payload:** 20 MiB serialized chart default (HTTP 413 naming the offending array index; `ITEM_MAX_BYTES`), with envelope headroom under the 32 MiB execution-topic cap.
- **Item count:** unlimited by default. Operators can opt into a cap with `BULK_MAX_ITEMS` (HTTP 400 above it).
- **Execution deadline:** the whole request shares one 300-second deadline regardless of size (`RPC_TIMEOUT_MS`; 60s without the chart env). Anything in front of HPS - ingress read timeouts, L4 idle timeouts - must sit above it; the chart configures Traefik accordingly, and the CLI raises Azure's 4-minute load-balancer idle timeout.
- **Concurrent large bodies:** each HPS replica admits at most 2 requests larger than 8 MiB at once (`LARGE_BODY_MAX_CONCURRENT` / `LARGE_BODY_THRESHOLD_BYTES`); excess requests are shed immediately with HTTP 429 + `Retry-After` instead of queueing in memory. Clients sending large batches should honor 429 with a short backoff.
- **Response amplification:** each chunk's response rides as a single Kafka message and must fit the 32 MiB execution-topic cap. Ordinary request chunks carry at most 2 MiB of input, leaving roughly 16x expansion headroom before the request fails with an explicit HTTP 413 rather than timing out.
- **Decision logs:** a single-item log retains the full request and response in one record. The logs topic and broker allow 64 MiB records, Vector fetches up to 128 MiB at a time, and each HPS/worker producer keeps a 128 MiB retry buffer.
- **Context batch requests** (`POST /api/v1/contexts/batch/{slug}`, self-hosted bulk mode) use the shared HTTP body limit, 128 MiB by chart default, with no separate Context record-count cap. Each stored instance permits 64 MiB of combined state and execution metadata. Database writes use chunks of 500 records by default (`CONTEXT_BATCH_DB_CHUNK`); larger requests continue across chunks. See [Bulk Ingestion](/contexts/bulk-ingestion) for execution, partial-failure, and capacity behavior.
...
The Rulebricks chart ships a Vector pod by default as the consumer of the `logs` topic. HPS API and worker pods produce structured decision-log entries to Kafka after each request completes (non-blocking, post-response). In the default persistent configuration, Vector writes every normalized record to both the app-facing ClickHouse MergeTree table and the durable object storage archive; custom S3, SIEM, or HTTP sinks can receive the same stream. PVC-less mode omits the local table write and serves date-bounded queries from the archive.

A few things to know when externalizing Kafka:

- **Consumer group.** Vector joins Kafka as `vector-consumers` with the topic prefix applied (default `com.rulebricks.vector-consumers`). The chart wires this automatically through the templated `vector-kafka-env` ConfigMap into `vector.customConfig.sources.kafka.group_id`. This prefixed name is the group ID to use in `kafka-consumer-groups.sh` commands and any ACL rules.
- **Consume-only.** In the default chart configuration Vector only reads from the `logs` topic; it does not produce back to Kafka. If your cluster enforces ACLs, the Vector principal needs `Read` and `Describe` on the prefixed `logs` topic with the prefixed group id (default `com.rulebricks.vector-consumers`), and no `Write`.
...
kafka-consumer-groups.sh --bootstrap-server <broker> \
  --describe --group com.rulebricks.vector-consumers
```
...
| Named-environment release pins           | No expiry (self-healing)         | Main app; HPS heals on miss    |
| Flow node results (API, SOAP, DB, vault) | 1–300s                           | Workers                        |

Redis is accessed using basic commands (`GET`, `SET` with `EX`, `EXPIRE`, `DEL`) plus one pub/sub channel: the main app publishes to `rulebricks:cache:invalidate` after rule, flow, API-key, or release changes, and HPS and worker pods subscribe to clear their in-process caches immediately. No Lua scripts, no streams. A vanilla Redis instance is all that's needed, with no clustering, persistence configuration, or eviction policy requirements, but the endpoint must support `PUBLISH`/`SUBSCRIBE`. If pub/sub is unavailable, pods log a warning and cache freshness degrades to TTL-based expiry only (release pins are additionally bounded by a short in-process TTL, so environment cutovers propagate within ~20 seconds even without pub/sub).

Losing Redis data is safe for correctness: release pins are rebuilt from the database automatically the next time each environment URL is requested, and `POST /api/v1/admin/releases/reconcile` rebuilds every pin for a workspace in one call (useful after a Redis migration or flush).
...
- **Named-environment URLs keep working but hit the database.** Release pins resolve from Postgres (with a short in-process cache) instead of Redis, adding a database round-trip on cache-miss requests.
...
| Named-environment URLs return 404                                            | No live release exists for that environment (Redis loss no longer causes this; pins self-heal from Postgres) | Check the release's status in the Releases tab; run `POST /api/v1/admin/releases/reconcile` to rebuild pins      |
