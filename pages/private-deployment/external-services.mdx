...
Set `max.message.bytes` to `8388608` (8 MiB) on `solution` and `solution-response`, matching the chart's in-cluster defaults. Request chunks are planner-capped at 2 MiB of input, so the headroom exists for **responses**: a worker's result rides as a single Kafka message and may legitimately grow to several times its chunk's input when rules expand payloads. A result that exceeds the cap fails the request with an explicit HTTP 413 rather than a timeout. The `logs` topic only needs `2097152` (2 MB) - decision-log records are sliced far below it.

Managed-service caveats for the 8 MiB profile:

- **AWS MSK Serverless** allows exactly 8 MiB (`max.message.bytes` maxes out at `8388608`), but its per-topic *default* is ~1 MiB - set the cap explicitly when creating the topics.
- **Azure Event Hubs** Standard and Premium cap events at 1 MB, which cannot carry the default large-payload profile; either lower the HPS payload envelope (`rulebricks.hps.limits` in the Helm values) to keep chunks and responses under 1 MB, or use a self-serve Dedicated cluster (20 MB events).
- **Confluent Cloud / Aiven / Redpanda** support 8 MiB per-topic caps on standard tiers.
...
- **Single payload:** 6 MiB serialized chart default (HTTP 413 naming the offending array index; `ITEM_MAX_BYTES`, which must stay below the topics' `max.message.bytes` minus envelope headroom).
...
- **Response amplification:** each chunk's response rides as a single Kafka message and must fit the topic's `max.message.bytes` (8 MiB chart default). Request chunks carry at most 2 MiB of input, so rules can expand outputs roughly 4x before a result is undeliverable - at which point the request fails with an explicit HTTP 413 identifying the response as too large, rather than timing out.
- **Context batch requests** (`POST /api/v1/contexts/batch/{slug}`, self-hosted bulk mode) accept 128 MiB bodies and cap records per request at 100,000 by chart default (HTTP 413 above it; `CONTEXT_BATCH_BODY_LIMIT_BYTES` / `CONTEXT_BATCH_MAX_ITEMS`). Database writes are chunked at 500 records per statement (`CONTEXT_BATCH_DB_CHUNK`). See [Bulk Ingestion](/contexts/bulk-ingestion) for the endpoint contract.
