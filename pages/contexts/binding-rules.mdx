...
This happens within a single API response. With `?include=execution_results,state,cascaded`, the `cascaded` array shows what ran, and each rule's outputs are also merged into the response's `state`:
...
By default, a bound rule or flow executes its **latest** published version. A new publish takes effect on the next context submission, including an identical retry. Publishing alone does not replay stored instances, and any vocabulary values it references resolve live at execution time.
