...
The green confirmation message, "This endpoint is active and can be used in production," signals that this version is now live. Any API call to the Release URL will execute this version of the rule. The transition from pending to released happens atomically in the database when the final required approval is given, ensuring there is no ambiguous intermediate state. Serving infrastructure is notified immediately; in the worst case (for example, if that notification is lost), each server re-checks its cached release target within about 20 seconds, which bounds how long an old version can continue to serve after a cutover.

Release targets are durable: the environment-to-version mapping lives in the database, not just in caches, so Release URLs keep resolving correctly across cache flushes, restarts, and infrastructure migrations.
