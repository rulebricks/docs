...
When the targeted version was published with tests in its test suite, a test results panel appears below the approval status: it shows the pass percentage and test counts captured at publish time, and calls out any critical tests that were failing by name. Versions published without tests simply omit the panel.
