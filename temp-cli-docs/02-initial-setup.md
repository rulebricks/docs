...
The wizard will prompt you for configuration options. You can also run in non-interactive mode by providing a configuration file or using flags (see [Configuration Reference](./04-configuration-reference.md)).
...
> **Note**: ARM-based instances are required (AWS Graviton, Azure Ampere, GCP Tau). The CLI will default to ARM instance types.
...
See [Vector Logging Setup](./08-vector-logging.md) for detailed configuration.
...
5. **Deploy** - See [Deployment Guide](./03-deployment.md)
...
For more help, see [Status & Troubleshooting](./07-status-troubleshooting.md).
