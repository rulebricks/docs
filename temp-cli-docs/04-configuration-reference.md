...
> **Note**: This configuration section exists in the schema but is not fully implemented. Use `monitoring.local.retention` for local mode retention instead.
...
> **Note**: This configuration section exists in the schema but is not currently implemented.
...
See [Vector Logging Setup](./08-vector-logging.md) for detailed examples.
...
> **Note**: Backup functionality is planned but not yet implemented. This configuration exists in the schema for future use but is not currently processed during deployment.
...
For now, you'll need to set up database backups manually. See [Upgrades & Maintenance](./06-upgrades-maintenance.md#backup-database) for manual backup options.
...
- [Initial Setup](./02-initial-setup.md) - Using the init wizard
- [Deployment Guide](./03-deployment.md) - Deploying your cluster
